...
class NerModule(object):
        ...
        for i in range(len(sequence)):
            for j in range(i + 2, len(sequence) + 1):
                if sequence[i:j] in entity_stop:
                    continue
                key = sequence[i:j]
                if key in self.kb.v2e_idx_dict:
    ...
    def getMentionFromHardMatch(
        ...
        """
        mentions = self.getPossibleMentions(sequence=sequence, max_str=max_str)
        values = self.getPossibleValues(sequence=sequence, max_str=max_str)
        ans = list(set(mentions + values))
        return ans

    def getMentionFromRules(self, sequence)->list:
        """
    ...
    def CombineNerResult(
            self,
            bert_ner_list:list=None,
            search_ner_list:list=None,
            rule_ner_list:list=None
    )->list:
        """
        ...
        for bert_ner in bert_ner_list:
            ...
            if bert_ner in search_ner_list or len(bert_ner) == 1:
                ...
                for search_ner in search_ner_list:
                    sim = editDistance(bert_ner, search_ner)
